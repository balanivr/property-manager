...
const columns = [
    ...
    { id: 'agent', numeric: false, hideOnMobile: true, disablePadding: false, label: 'Created By' },
    { id: 'active', numeric: true, hideOnMobile: true, disablePadding: false, label: 'Active' },
    { id: 'approve', numeric: true, hideOnMobile: false, disablePadding: false, label: '' },
];
        ...
        return (
            <TableHead>
                <TableRow>
                    ...
                    {columns.map(
                        row => (
                            <Hidden xsDown={row.hideOnMobile}>
                                <TableCell
                                    key={row.id}
...
class RepresentativeViewingTable extends React.Component {
    state = {
        order: 'asc',
        orderBy: 'name',
        selected: [],
    ...
    viewRow = (representative) => () => {
        console.log('Open view');
        this.props.onRowView && this.props.onRowView(representative);
    };
    ...
    handleApprove = (event, representative) => {
        event.stopPropagation();
        this.props.approveAction(representative);
    }
                                ...
                                .map(representative => {
                                    ...
                                    return (
                                        <TableRow
                                            ...
                                            selected={isSelected}
                                            onClick={this.viewRow(representative)}
                                            style={{
                                                cursor: 'pointer',
                                        ...
                                        >
                                            ...
                                            <Hidden xsDown={true}>
                                                <TableCell component="th" padding="none">
                                                    { representative.agent.name ? representative.agent.name : '' }
                                                </TableCell>
                                            </Hidden>
                                            <Hidden xsDown={true}>
                                                <TableCell component="th" padding="none">
                                            ...
                                            <Hidden xsDown={false}>
                                                <TableCell component="th" padding="none" >
                                                    <Tooltip
                                                        title={"Approve " + representative.name}
                                                        // placement={'bottom-end'}
                                                        enterDelay={300}
                                                    >
                                                        <IconButton onClick={e => this.handleApprove(e, representative)} color="secondary">
                                                            <DoneIcon />
                                                        </IconButton>
                                                    </Tooltip>
                                                </TableCell>
...
export default withStyles(styles, { withTheme: true })(RepresentativeViewingTable);