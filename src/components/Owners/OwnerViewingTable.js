...
const columns = [
    ...
    { id: 'addresses', numeric: true, disablePadding: false, label: 'Addresses' },
    { id: 'properties', numeric: true, disablePadding: false, label: 'Properties' },
    { id: 'agent', numeric: false, disablePadding: false, label: 'Created By' },
    { id: 'active', numeric: true, disablePadding: false, label: 'Active' },
    { id: 'approve', numeric: true, disablePadding: false, label: '' },
];
...
class OwnerTable extends React.Component {
    ...
    viewRow = (owner) => () => {
        ...
        this.props.onRowView && this.props.onRowView(owner);
    };
    ...
    handleApprove = (event, owner) => {
        event.stopPropagation();
        this.props.approveAction(owner);
    }
        ...
        return (
            <React.Fragment>
                <EnhancedTableToolbar 
                    ...
                    searchOwner={this.props.searchOwner}
                />
                <div className={classes.tableWrapper}>
                    <Table className={classes.table} aria-labelledby="owners">
                      <EnhancedTableHead
                        ...
                        />
                        <TableBody>
                            {stableSort(owners, getSorting(order, orderBy))
                                .slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
                                .map(owner => {
                                    const isSelected = this.isSelected(owner.id);
                                    let ownerStatus = owner.status && owner.status === ACTIVE_STATES.ACTIVE;
                                    ...
                                    return (
                                        <TableRow
                                            ...
                                            key={owner.id}
                                            selected={isSelected}
                                            onClick={this.viewRow(owner)}
                                            style={{
                                                cursor: 'pointer',
                                                background: owner.searchFound ? '#b4f673' : ''
                                            }}
                                        >
                                            <TableCell padding="checkbox">
                                                <Checkbox
                                                    checked={isSelected}
                                                    onClick={event => this.handleSelect(event, owner.id)}
                                                />
                                            </TableCell>
                                            ...
                                            <TableCell component="th" padding="none">
                                                <Switch defaultChecked={ownerStatus} onChange={this.handleStatusChange(owner)}/>
                                            </TableCell>
                                            <TableCell component="th" padding="none" >
                                                <Tooltip
                                                    ...
                                                    enterDelay={300}
                                                >
                                                    <IconButton onClick={e => this.handleApprove(e, owner)} color="secondary">
                                                        <DoneIcon />
                                                    </IconButton>
                                                </Tooltip>
                                            </TableCell>
...
export default withStyles(styles, { withTheme: true })(OwnerTable);